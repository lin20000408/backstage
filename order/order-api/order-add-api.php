...
if (!empty($_POST['total_cost'])) {
  ...
  if ($isPass) {
    #避免 SQL injection
    $sql = "INSERT INTO `midterm`.`order`
    (`coupon_id`,`shipping_method`,`payment_method`,`total_cost`,`order_status`,`order_creation_time`,`receiver`,`receiver_address`,`receiver_phone`)
    VALUES (
      ?,?,?,?,?,?,?,?,?
    )";
    ...
    $stmt->execute([$coupon_id, $_POST['shipping_method'], $_POST['payment_method'], $_POST['total_cost'], $_POST['order_status'], $currentDateTime, $_POST['receiver'], $_POST['receiver_address'], $_POST['receiver_phone']]);
    ...
    // ID 插入訂單詳情
    $sql2 = "INSERT INTO `midterm`.`order_details`
    (`order_id`, `product_name`,`product_size`,`product_color`,`product_amount`,`price`,`product_amount_total`,`product_reviews_id`,`user_id`)
    VALUES
    (?,?,?,?,?,?,?,?,?)
    ";
