...
?>
<?php $title = '此商品評價列表';
$pageName = 'product-review-specific-id';
...
<div class="d-flex w-100 h-100">
    ...
    <div class="container">
        ...
        $t_sql = "SELECT COUNT(1) FROM (SELECT order_details.order_id,
                    ...
                    ON order_details.product_reviews_id = product_reviews.id  WHERE product_id=$id) AS subquery
                    ";
        ...
        if ($totalRows > 0) {
            ...
            $sql = sprintf(" SELECT order_details.order_id,
                        ...
                        FROM `midterm`.product_reviews
                        INNER JOIN `midterm`.order_details 
                        ON order_details.product_reviews_id = product_reviews.id  WHERE product_id=$id ORDER BY id ASC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
