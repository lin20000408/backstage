...
$title = '有照片的評價列表';
$pageName = 'product-review-groupBy-photo';
...
<div class="d-flex w-100 h-100">
    ...
    <div class="container">
        ...
        $t_sql = "SELECT COUNT(1) FROM (SELECT order_details.order_id,
                    ...
                    ON order_details.product_reviews_id = product_reviews.id WHERE product_reviews.photo  <>'' ) AS subquery
                    ";
        ...
        if ($totalRows > 0) {
            ...
            $sql = sprintf(" SELECT order_details.order_id,
                        ...
                        FROM `midterm`.product_reviews
                        INNER JOIN `midterm`.order_details 
                        ON order_details.product_reviews_id = product_reviews.id  WHERE product_reviews.photo  <>''  ORDER BY id ASC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
            $rows  = $pdo->query($sql)->fetchAll();
        } ?>

        <!-- 可參考下面的table 統一格式 -->
        <div class="container">
            <h2 class="text-center">含有照片的評價</h2>
