...
$title = '一星評價列表';
$pageName = 'product-review-star-one';
...
<div class="d-flex w-100 h-100">
    ...
    <div class="container">
        ...
        $t_sql = "SELECT COUNT(1) FROM (SELECT order_details.order_id,
                    ...
                    ON order_details.product_reviews_id = product_reviews.id WHERE product_reviews.stars='1' ) AS subquery
                    ";
        ...
        if ($totalRows > 0) {
            ...
            $sql = sprintf(" SELECT order_details.order_id,
                        ...
                        FROM `midterm`.product_reviews
                        INNER JOIN `midterm`.order_details 
                        ON order_details.product_reviews_id = product_reviews.id WHERE product_reviews.stars='1'  ORDER BY id ASC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
            $rows  = $pdo->query($sql)->fetchAll();
        } ?>

        <h2 class="text-center">一星商品評價</h2>
        <!-- 可參考下面的table 統一格式 -->
        <div class="container">
