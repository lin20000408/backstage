...
$title = '關鍵字搜尋評價列表';
$pageName = 'product-review-search';
...
<div class="d-flex w-100 h-100">
    ...
    <div class="container">
        ...
        $ids = isset($_GET['id']) ? array_map('intval', explode(',', $_GET['id'])) : [];
        ...
        $t_sql = "SELECT COUNT(1) FROM (SELECT order_details.order_id,
                    ...
                    ON order_details.product_reviews_id = product_reviews.id WHERE product_reviews.id IN('" . implode("','", $ids) . "')) AS subquery
                    ";
        ...
        if ($totalRows > 0) {
            ...
            $sql = sprintf(" SELECT order_details.order_id,
                        ...
                        FROM `midterm`.product_reviews
                        INNER JOIN `midterm`.order_details 
                        ON order_details.product_reviews_id = product_reviews.id WHERE product_reviews.id IN('" . implode("','", $ids) . "') ORDER BY id ASC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
