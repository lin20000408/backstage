...
<main class="me-5 vh-100 position-fixed" id="left-corner">
  <div class="d-flex flex-column p-3 text-white bg-dark vh-100" style="width: 400px">
    ...
    <ul class="nav nav-pills flex-column mb-auto">
      <li>
        <a class="nav-link text-white <?= $isAdminPage ? 'active' : '' ?>" href=<?= isset ($_SESSION['admin']) ? "./admin-login.php" : 'javascript:void(0);' ?> onclick=<?= isset($_SESSION['admin']) ? '' : 'showAlert();' ?>>
          後臺管理
        </a>
      </li>
      <li>
    ...
    <?php if (isset ($_SESSION['admin'])): ?>
      <div class="dropdown">
        <a href="#" class="d-flex align-items-center text-white text-decoration-none dropdown-toggle" id="dropdownUser1"
          ...
          <img src=<?= $row['picture'] ?> alt="" width="32" height="32"
            class="rounded-circle me-2" id="loginPicture" />
          <strong>
    ...
    <div class="modal fade" id="changePictureModal" tabindex="-1" aria-labelledby="changePictureModalLabel"
      aria-hidden="true" id="changeBigHead">
